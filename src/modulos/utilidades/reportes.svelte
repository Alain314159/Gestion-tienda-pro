...
<script>
  ...
  import { listar, guardar, leerConfig } from '../../core/db.js';
  import { bus } from '../../core/bus.js';
  import { avisar, confirmar } from '../../core/state.svelte.js';
  import { n, m, fmt, fmtFecha, genId, generarReporte, clean, nowLocal } from '../../core/util.js';
  ...
  let cierresOrdenados = $derived(
    cierres.slice().sort((a, b) => new Date(b.fechaCierre) - new Date(a.fechaCierre))
  );
  ...
  function generar() {
    ...
    rep = generarReporte({ ventas, compras, ajustes }, repInicio, repFin);
    if (rep.error) {
      avisar(rep.error, 'bad');
      rep = null;
    }
  }
  function exportarCSV() {
    ...
    a.href = url;
    a.download = 'reporte_' + repInicio + '_' + repFin + '.csv';
    a.click();
  ...
  async function cerrarPeriodo() {
    const ok = await confirmar(
      'Cerrar periodo',
      'Esto reinicia los contadores del dashboard. Continuar?'
    );
    if (!ok) return;
    ...
    const gastos = m(
      (await listar('gastosOp'))
        .filter((g) => new Date(g.fecha) >= new Date(periodoInicio))
        .reduce((s, g) => s + n(g.monto), 0)
    );
    const neta = m(bruta - mermas - gastos);
    const cierre = {
      id: genId('cr'),
      fechaCierre: nowLocal().iso,
...
<div class="modulo">
  ...
  <div class="bg-card rounded-[var(--radius-lg)] p-4 shadow-[var(--color-shadow)] mb-3">
    <div class="flex items-center gap-2 font-extrabold text-primary mb-3">
      <Icono nombre="calendar" size={18} /> Reporte por periodo
    </div>
    ...
    <button
      ...
      onclick={generar}
    >
      <Icono nombre="chart" size={16} color="#fff" /> Generar Reporte
    </button>
    {#if rep}
      <div class="bg-background rounded-[var(--radius-md)] p-3 mt-3 space-y-0">
        <div class="flex justify-between items-center gap-2 py-2 border-b border-border text-sm">
          <div class="flex items-center gap-2">
            <div class="w-2 h-2 rounded-full bg-success"></div>
            <span>Ingresos</span>
          </div>
          <span class="text-success font-bold">{fmt(rep.ingresos)}</span>
        </div>
        <div class="flex justify-between items-center gap-2 py-2 border-b border-border text-sm">
          <div class="flex items-center gap-2">
          ...
          <span class="text-danger font-bold">-{fmt(rep.cogs)}</span>
        </div>
        <div
          class="flex justify-between items-center gap-2 py-2 border-b border-border text-sm font-bold bg-primary/5 rounded-lg px-3 mt-1"
        >
          <span>Ganancia bruta</span><span class="text-primary">{fmt(rep.bruta)}</span>
        </div>
        <div class="flex justify-between items-center gap-2 py-2 border-b border-border text-sm">
          <div class="flex items-center gap-2">
            <div class="w-2 h-2 rounded-full bg-danger"></div>
          ...
          <span class="text-danger font-bold">-{fmt(rep.mermas)}</span>
        </div>
        <div
          class="flex justify-between items-center gap-2 py-2 border-b border-border text-sm font-bold"
        >
          <span>Ganancia neta</span><span class="text-success">{fmt(rep.neta)}</span>
        </div>
        <div class="flex justify-between items-center gap-2 py-2 border-b border-border text-sm">
          <span>Margen bruto</span><span class="font-bold">{rep.margenB}%</span>
        </div>
        <div class="flex justify-between items-center gap-2 py-2 text-sm font-bold">
          <span>Margen neto</span><span class="font-bold">{rep.margenN}%</span>
        </div>
        <div class="flex justify-between items-center gap-2 py-2 text-sm">
          <span>Ventas</span><span class="font-bold">{rep.numVentas}</span>
        </div>
      </div>
      <button
        class="w-full py-2.5 rounded-[var(--radius-md)] border border-border bg-transparent text-text font-extrabold text-sm mt-2 active:scale-[0.97] transition-transform"
        onclick={exportarCSV}
      >
        <Icono nombre="export" size={14} /> Exportar CSV
      </button>
    {/if}
  </div>
  <!-- Cierre de periodo -->
  <div class="bg-card rounded-[var(--radius-lg)] p-4 shadow-[var(--color-shadow)] mb-3">
    <div class="flex items-center gap-2 font-extrabold text-danger mb-3">
      <Icono nombre="lock" size={18} /> Cierre de periodo
    </div>
    <p class="text-sm text-muted mb-3">
      Cierra el periodo actual para reiniciar los contadores del dashboard. El historial se
      conserva.
    </p>
    <button
    ...
    >
      <Icono nombre="lock" size={16} color="#fff" /> Cerrar Periodo
    </button>
  </div>
  <!-- Historial de cierres -->
  <div class="bg-card rounded-[var(--radius-lg)] p-4 shadow-[var(--color-shadow)]">
    <div class="flex items-center gap-2 font-extrabold text-primary mb-3">
      <Icono nombre="history" size={18} /> Historial de cierres
    </div>
