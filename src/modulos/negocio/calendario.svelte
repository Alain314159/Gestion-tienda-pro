...
<script>
  ...
  let ventasDiaSel = $derived(
    diaSel
      ? ventasDelDia(
          ventas,
          `${anio}-${String(mes + 1).padStart(2, '0')}-${String(diaSel).padStart(2, '0')}`
        )
      : []
  );
  async function recargar() {
    ventas = await listar('ventas');
  }
  onMount(() => {
    recargar();
...
<div class="modulo">
  <div class="bg-card rounded-[var(--radius-lg)] p-4 shadow-[var(--color-shadow)] mb-3">
    <div class="flex items-center justify-between mb-4">
      <button
        class="w-10 h-10 rounded-full bg-background border border-border flex items-center justify-center active:scale-[0.92] transition-transform"
        onclick={() => {
          mes--;
          if (mes < 0) {
            mes = 11;
    ...
    <div class="grid grid-cols-7 gap-1">
      {#each calendario() as d}
        ...
        {:else}
          ...
          <button
            class="aspect-square rounded-xl flex flex-col items-center justify-center text-sm font-bold transition-all relative {diaSel ===
            d
              ? 'bg-primary text-white shadow-lg scale-105'
              : esHoy
                ? 'bg-primary/10 text-primary border-2 border-primary'
                : 'bg-background text-text hover:bg-border/50'}"
            onclick={() => (diaSel = diaSel === d ? null : d)}
          >
            <span>{d}</span>
            {#if info}
              <span class="text-[0.6rem] {diaSel === d ? 'text-white/80' : 'text-success'}"
                >{fmt(info.ventas).slice(0, 6)}</span
  ...
  {#if diaSel}
    <div
      class="bg-card rounded-[var(--radius-lg)] p-4 shadow-[var(--color-shadow)] animate-fade-up"
    >
      <div class="flex items-center gap-2 font-extrabold text-primary mb-3">
        <Icono nombre="cart" size={18} /> Ventas del {diaSel}/{mes + 1}/{anio}
      </div>
      {#if ventasDiaSel.length === 0}
        <div class="text-center text-muted py-4 text-sm">Sin ventas</div>
      {:else}
        <div class="text-sm text-muted mb-2">
          {ventasDiaSel.length} ventas · Total {fmt(
            ventasDiaSel.reduce((s, v) => s + n(v.total), 0)
          )}
        </div>
        {#each ventasDiaSel as v}
          <div class="flex justify-between items-center py-2.5 border-b border-border text-sm">
            <span class="truncate flex-1 font-medium"
              >{v.items.map((i) => i.nombre).join(', ')}</span
            > <span class="font-bold flex-shrink-0">{fmt(v.total)}</span>
          </div>
        {/each}
      {/if}
    </div>
