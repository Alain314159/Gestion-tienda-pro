...
<script>
  ...
  const valInv = $derived(
    toNumber(
      sum(
        lotes.filter((l) => n(l.cantidadInicial) - n(l.cantidadVendida) > 0),
        (l) => mul(n(l.cantidadInicial) - n(l.cantidadVendida), l.costo)
      )
    )
  );
  const unidadesTotal = $derived(
    toNumber(
      sum(
        lotes.filter((l) => n(l.cantidadInicial) - n(l.cantidadVendida) > 0),
        (l) => n(l.cantidadInicial) - n(l.cantidadVendida)
      )
    )
  );
  ...
  async function recargar() {
    [productos, variantes, lotes, ajustes] = await Promise.all([
      listar('productos'),
  ...
  async function registrarAjuste() {
    ...
    try {
      ...
      form = { productoId: '', varianteId: '', cantidad: '', motivo: '', costoSobrante: '' };
    } catch (e) {
...
<div class="modulo">
  ...
  <div class="bg-card rounded-[var(--radius-lg)] p-4 shadow-[var(--color-shadow)] mb-3">
      ...
      {#if selProd}
        <div class="bg-primary/5 rounded-xl p-3 mb-3 flex justify-between items-center">
          <div>
            ...
            <div class="font-bold text-sm">{selProd.nombre}</div>
          </div>
          <button
        ...
        <div class="mb-2">
          ...
          {#each variantesDeProducto(form.productoId) as v}
            ...
            {@const activa = form.varianteId === v.id}
            <button
              class="w-full flex justify-between items-center px-3 py-2.5 text-left border rounded-[var(--radius-md)] mb-1.5 text-sm cursor-pointer transition-colors {activa
                ? 'bg-primary/10 border-primary'
                : 'border-border/50 hover:bg-background'}"
              onclick={() => (form.varianteId = v.id)}
            >
    ...
    {#if form.varianteId}
      ...
      >
        ...
        {procesando ? 'Procesando...' : 'Registrar Ajuste'}
      </button>
    {/if}
  </div>
  ...
  <div class="bg-card rounded-[var(--radius-lg)] p-3 shadow-[var(--color-shadow)] mb-3">
    <div class="flex items-center gap-2 font-extrabold text-primary mb-3 px-1 pt-1">
      <Icono nombre="package" size={18} /> Inventario por variante
    </div>
    {#if variantes.length === 0}
      <div class="text-center text-muted py-6 text-sm">Sin inventario</div>
    {:else}
      {#each productos.filter((p) => !p.archivado) as p}
        {@const vars = variantesDeProducto(p.id)}
        {#if vars.length > 0}
          <div
            class="bg-card rounded-[var(--radius-lg)] shadow-[var(--color-shadow)] mb-2 overflow-hidden border border-border/30"
          >
            <button
            ...
            >
              <div class="min-w-0 flex-1">
                <div class="font-extrabold text-primary text-sm">{p.nombre}</div>
                <div class="text-xs text-muted">{vars.length} variantes</div>
              </div>
              <Icono nombre="chevron" size={16} class={expandido[p.id] ? 'rotate-180' : ''} />
            </button>
            {#if expandido[p.id]}
                  ...
                  <div class="mb-2">
                    <div class="flex justify-between items-center py-2 px-3 bg-card rounded-lg border border-border/20">
                      <div>
                        ...
                        {#if v.esCaja}
                          {@const dec = ConversionService.descomponerStock(s, v.unidadesPorCaja)}
                          <span class="text-xs text-primary ml-1"
                            >({dec.cajas} cajas + {fmtCant(dec.unidades)} unidades)</span
                          >
                        {/if}
                        <div class="text-xs text-muted mt-0.5">
                          Stock: <b class="text-text">{fmtCant(s)} {v.unidad || ''}</b> · Valor:
                          <b class="text-primary">{fmt(val)}</b>
                        </div>
                      </div>
                    </div>
                    {#if lotesV.length > 0}
  ...
  <div class="bg-card rounded-[var(--radius-lg)] p-4 shadow-[var(--color-shadow)]">
    <div class="flex items-center gap-2 font-extrabold text-primary mb-3">
      <Icono nombre="history" size={18} /> Ultimos ajustes
    </div>
    ...
    {:else}
      {#each ajustesRecientes as a}
        ...
        >
          <div class="min-w-0 flex-1">
            <div class="flex items-center gap-2">
              <div class="w-2 h-2 rounded-full {a.cantidad < 0 ? 'bg-danger' : 'bg-success'}"></div>
              <span class="font-bold text-sm">{a.productoNombre}</span>
            </div>
            <div class="text-xs text-muted ml-4">{fmtFH(a.fecha)} · {a.motivo}</div>
          </div>
          <div class="text-right flex-shrink-0">
