...
<script>
  ...
  import { listar, listarPaginado, leerConfig } from '../../core/db.js';
  ...
  const ventasPeriodoArr = $derived(
    ventas.filter((v) => !v.anulada && new Date(v.fecha) >= new Date(periodoInicio))
  );
  const comprasPeriodoArr = $derived(
    compras.filter((c) => !c.anulada && new Date(c.fecha) >= new Date(periodoInicio))
  );
  ...
  const margen = $derived(
    ventasPeriodo > 0 ? ((gananciaNeta / ventasPeriodo) * 100).toFixed(1) : '0.0'
  );
  ...
  const maxGanTop = $derived(topRent.length > 0 ? Math.max(...topRent.map((p) => p.gan)) : 1);
  ...
  async function recargar() {
    [productos, variantes, lotes, ventas, compras, ajustes, movCaja, cierres, capital, retiros] =
      await Promise.all([
        listar('productos'),
  ...
  onMount(() => {
    ...
    let ro, resizeTimeout;
    if (chartCanvas && 'ResizeObserver' in window) {
      ro = new ResizeObserver(() => {
        clearTimeout(resizeTimeout);
        resizeTimeout = setTimeout(() => {
          if (chartCanvas) chartCanvas.width = chartCanvas.offsetWidth;
        }, 150);
      });
...
<div class="modulo">
  {#if agotados.length > 0 || bajoStock.length > 0}
    <div
      class="bg-card rounded-[var(--radius-lg)] p-4 shadow-[var(--color-shadow)] mb-3 border-l-4 border-warning"
    >
  ...
  {/if}
  <div
    class="bg-gradient-to-br from-primary to-[#1e3a8a] text-white rounded-[var(--radius-lg)] p-5 text-center mb-3 shadow-[var(--color-shadow)] relative overflow-hidden"
  >
    <div
    ...
    <div class="relative">
      <div class="flex items-center justify-center gap-1.5 text-xs opacity-90 mb-1">
        <Icono nombre="wallet" size={14} color="#fff" /> Efectivo en Caja
      </div>
      <div class="text-3xl font-extrabold my-0.5">{fmt(saldo)}</div>
      <div class="text-xs opacity-85">
        Inventario: {fmt(valInv)} · Desde {fmtFecha(periodoInicio)}
      </div>
    </div>
  </div>
  ...
  <div class="bg-card rounded-[var(--radius-lg)] p-4 shadow-[var(--color-shadow)] mb-3">
    <div class="flex items-center gap-2 font-extrabold text-primary mb-3">
      <Icono nombre="chart" size={18} /> Ventas vs Ganancia (6 meses)
    </div>
    ...
    <div class="flex justify-center gap-4 mt-2">
      <div class="flex items-center gap-1 text-xs text-muted">
        <span class="w-3 h-3 rounded-sm bg-primary"></span> Ventas
      </div>
      <div class="flex items-center gap-1 text-xs text-muted">
        <span class="w-3 h-3 rounded-sm bg-success"></span> Ganancia
      </div>
    </div>
  </div>
  <div class="bg-card rounded-[var(--radius-lg)] p-4 shadow-[var(--color-shadow)] mb-3">
    <div class="flex items-center gap-2 font-extrabold text-primary mb-3">
      <Icono nombre="diamond" size={18} /> Top rentables del mes
    </div>
    {#if topRent.length === 0}
    ...
    {:else}
      {#each topRent as p, i}
        <div class="flex items-center gap-3 py-2.5 border-b border-border last:border-0">
          ...
          >
            <span class="text-xs font-extrabold text-primary">{i + 1}</span>
          </div>
          <div class="flex-1 min-w-0">
            <div class="text-sm font-medium truncate">{p.nombre}</div>
            <div class="w-full h-1.5 bg-background rounded-full mt-1 overflow-hidden">
              <div
                class="h-full bg-success rounded-full"
                style="width: {(p.gan / maxGanTop) * 100}%"
              ></div>
            </div>
          </div>
          <div class="text-success font-bold text-sm flex-shrink-0">{fmt(p.gan)}</div>
        </div>
      {/each}
    {/if}
  </div>
