...
<div class="modulo">
  ...
  <div class="bg-card rounded-[var(--radius-lg)] p-4 shadow-[var(--color-shadow)] mb-3">
    <div class="flex items-center gap-2 font-extrabold text-primary mb-3">
      <Icono nombre="calendar" size={18} /> Fecha del cuadre
    </div>
    <input
      type="date"
  ...
  <div class="grid grid-cols-2 gap-2.5 mb-3">
    ...
    <div class="bg-danger/5 rounded-xl p-3 text-center">
      <div class="text-[0.65rem] text-muted font-bold">COSTO</div>
  ...
  <div class="bg-card rounded-[var(--radius-lg)] p-4 shadow-[var(--color-shadow)] mb-3">
    ...
    <button
      ...
      onclick={exportarPDF}
    >
      <Icono nombre="fileText" size={16} color="#fff" /> Exportar PDF
    </button>
  </div>
  <!-- Gastos -->
  <div class="bg-card rounded-[var(--radius-lg)] p-4 shadow-[var(--color-shadow)] mb-3">
    <div class="flex items-center gap-2 font-extrabold text-warning mb-3">
      <Icono nombre="receipt" size={18} /> Gastos operativos
    </div>
    ...
    {/if}
    <button
      class="w-full py-2.5 mt-3 rounded-[var(--radius-md)] border border-border bg-transparent text-text font-bold text-sm active:scale-[0.97] transition-transform"
      onclick={agregarGasto}
    >
      + Agregar gasto
    </button>
  </div>
  <!-- Socios con barras visuales -->
  <div class="bg-card rounded-[var(--radius-lg)] p-4 shadow-[var(--color-shadow)] mb-3">
    <div class="flex items-center gap-2 font-extrabold text-purple mb-3">
      <Icono nombre="users" size={18} /> Distribucion entre socios
    </div>
    {#if distribucion.length === 0}
      <div class="text-center text-muted py-4 text-sm">Sin socios configurados</div>
    {:else}
        ...
        <div class="py-3 border-b border-border last:border-0">
          <div class="flex justify-between items-center mb-1">
            ...
            <div class="text-right">
              <div class="font-extrabold text-success text-sm">{fmt(s.monto)}</div>
  ...
  <div class="bg-card rounded-[var(--radius-lg)] p-4 shadow-[var(--color-shadow)]">
    <div class="flex items-center gap-2 font-extrabold text-primary mb-3">
      <Icono nombre="cart" size={18} /> Ventas del dia ({vDia.length})
    </div>
    {#if vDia.length === 0}
