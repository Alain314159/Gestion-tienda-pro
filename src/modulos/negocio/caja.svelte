...
<script>
  ...
  import { toNumber, sum, sumWhere } from '../../core/Money.js';
  ...
  let aportesTotal = $derived(toNumber(sum(capital, 'monto')));
  let ventasTotal = $derived(toNumber(sumWhere(ventas, (v) => !v.anulada, 'total')));
  let comprasTotal = $derived(toNumber(sumWhere(compras, (c) => !c.anulada, 'total')));
  let retirosTotal = $derived(toNumber(sum(retiros, 'monto')));
  let arqueoNeto = $derived(
    toNumber(
      sumWhere(
        movCaja,
        (mv) => mv.concepto && mv.concepto.toLowerCase().includes('arqueo'),
        (mv) => (mv.tipo === 'ingreso' ? mv.monto : -mv.monto)
      )
    )
  );
  let movs = $derived(movimientosCaja({ cfg, capital, ventas, compras, retiros, movCaja }));
  let movsPaginados = $derived(
    movs.slice((paginaMovs - 1) * itemsPorPagina, paginaMovs * itemsPorPagina)
  );
  let totalPaginasMovs = $derived(Math.max(1, Math.ceil(movs.length / itemsPorPagina)));
  let arqueoPreview = $derived(
    (() => {
      if (arqueoForm.monto === '' || arqueoForm.monto === null || arqueoForm.monto === undefined)
        return null;
  ...
  async function registrarArqueo() {
    ...
    try {
      ...
      if (Math.abs(diff) < 0.01) {
        ...
        avisar(
          (diff > 0 ? 'Sobrante ' : 'Faltante ') + fmt(Math.abs(diff)),
          diff > 0 ? 'warn' : 'bad'
        );
      }
    } catch (e) {
...
<div class="modulo">
  ...
  <div
    class="{saldo < 0
      ? 'bg-gradient-to-br from-danger to-[#7f1d1d]'
      : 'bg-gradient-to-br from-primary to-[#1e3a8a]'} text-white rounded-[var(--radius-lg)] p-5 text-center mb-3 shadow-[var(--color-shadow)] relative overflow-hidden"
  >
    <div
    ...
    <div class="relative">
      <div class="flex items-center justify-center gap-1.5 text-xs opacity-90 mb-1">
        <Icono nombre="wallet" size={14} color="#fff" /> Saldo en Caja
      </div>
  ...
  <div class="bg-card rounded-[var(--radius-lg)] p-4 shadow-[var(--color-shadow)] mb-3">
    <div class="flex items-center gap-2 font-extrabold text-primary mb-3">
      <Icono nombre="chart" size={18} /> Desglose
    </div>
    <div class="space-y-0">
      <div class="flex justify-between items-center gap-2 py-2.5 border-b border-border text-sm">
        ...
        <span class="text-success font-bold">+{fmt(cfg.capitalInicial || 0)}</span>
      </div>
      <div class="flex justify-between items-center gap-2 py-2.5 border-b border-border text-sm">
        <div class="flex items-center gap-2">
          <div class="w-2 h-2 rounded-full bg-success"></div>
        ...
        <span class="text-success font-bold">+{fmt(aportesTotal)}</span>
      </div>
      <div class="flex justify-between items-center gap-2 py-2.5 border-b border-border text-sm">
        <div class="flex items-center gap-2">
          <div class="w-2 h-2 rounded-full bg-success"></div>
        ...
        <span class="text-success font-bold">+{fmt(ventasTotal)}</span>
      </div>
      <div class="flex justify-between items-center gap-2 py-2.5 border-b border-border text-sm">
        <div class="flex items-center gap-2">
        ...
        <span class="text-danger font-bold">-{fmt(comprasTotal)}</span>
      </div>
      <div class="flex justify-between items-center gap-2 py-2.5 border-b border-border text-sm">
        <div class="flex items-center gap-2">
          <div class="w-2 h-2 rounded-full bg-danger"></div>
        ...
        <span class="text-danger font-bold">-{fmt(retirosTotal)}</span>
      </div>
      <div class="flex justify-between items-center gap-2 py-2.5 border-b border-border text-sm">
        <div class="flex items-center gap-2">
          <div class="w-2 h-2 rounded-full {arqueoNeto >= 0 ? 'bg-success' : 'bg-danger'}"></div>
          <span>Ajustes arqueo</span>
        </div>
        <span class="{arqueoNeto >= 0 ? 'text-success' : 'text-danger'} font-bold"
          >{arqueoNeto >= 0 ? '+' : ''}{fmt(arqueoNeto)}</span
        >
      </div>
      <div class="flex justify-between items-center gap-2 py-3 mt-1 bg-primary/5 rounded-xl px-3">
        <span class="font-extrabold text-sm">= SALDO</span>
        <span class="text-primary font-extrabold text-base">{fmt(saldo)}</span>
      </div>
    </div>
  </div>
  <!-- Arqueo -->
  <div class="bg-card rounded-[var(--radius-lg)] p-4 shadow-[var(--color-shadow)] mb-3">
    <div class="flex items-center gap-2 font-extrabold text-primary mb-3">
      <Icono nombre="search" size={18} /> Arqueo de Caja
    </div>
    <div
      class="bg-primary/10 border-l-4 border-primary rounded-[var(--radius-md)] p-3 mb-3 text-sm text-primary"
    >
      Cuenta el dinero fisico y escribe lo que tienes. Si es MENOR que el sistema = faltante; si es
      MAYOR = sobrante.
    </div>
    <div
      class="flex justify-between items-center gap-2 py-1 text-sm mb-2 bg-background rounded-lg px-3"
    >
      <span class="text-muted">El sistema dice:</span>
      <b class="text-primary text-base">{fmt(saldo)}</b>
    </div>
    <input
      ...
      inputmode="decimal"
      step="0.01"
      placeholder="Monto que contaste fisicamente"
      bind:value={arqueoForm.monto}
    />
    {#if arqueoPreview}
      <div
        class="rounded-[var(--radius-md)] p-3 mb-2 font-extrabold text-sm {arqueoPreview.tipo ===
        'cuadre'
          ? 'bg-success/10 text-success'
          : arqueoPreview.tipo === 'sobrante'
            ? 'bg-warning/10 text-warning'
            : 'bg-danger/10 text-danger'}"
      >
        Tu cuentas: {fmt(arqueoPreview.fisico)} · Diferencia: {#if arqueoPreview.tipo === 'cuadre'}Cuadre
          perfecto ✓
  ...
  <div class="bg-card rounded-[var(--radius-lg)] p-4 shadow-[var(--color-shadow)]">
    <div class="flex items-center gap-2 font-extrabold text-primary mb-3">
      <Icono nombre="history" size={18} /> Movimientos recientes
    </div>
    ...
    {:else}
      {#each movsPaginados as mv}
        <div
          class="flex justify-between items-center gap-2 py-2.5 border-b border-border {esCerrado(
            mv.fecha
          )
            ? 'opacity-60'
            : ''}"
        >
          ...
          <div class="flex items-center gap-1 flex-shrink-0">
            {#if esCerrado(mv.fecha)}
              ...
              >
            {:else}
              <b class={mv.tipo === 'ingreso' ? 'text-success' : 'text-danger'}
                >{mv.tipo === 'ingreso' ? '+' : '-'}{fmt(mv.monto)}</b
              >
            {/if}
          </div>
        </div>
      ...
      {#if totalPaginasMovs > 1}
        <div class="flex justify-center items-center gap-2 mt-3 text-sm">
          <button
            ...
            onclick={() => paginaMovs--}
            disabled={paginaMovs <= 1}>←</button
          > <span class="text-muted">{paginaMovs} / {totalPaginasMovs}</span>
          <button
            class="px-3 py-1 rounded-md border border-border bg-card disabled:opacity-30"
            onclick={() => paginaMovs++}
            disabled={paginaMovs >= totalPaginasMovs}>→</button
          >
        </div>
      {/if}
    {/if}
