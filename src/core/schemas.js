 ...
 *  Centraliza todas las validaciones de formularios de la app
 *  Reemplaza validaciones manuales dispersas por schemas tipados y robustos
 *  ================================================================ */
import { z } from 'zod';
...
// ID valido
const idSchema = z.string().min(1, { message: 'ID invalido' });
...
export const varianteSchema = z.object({
  ...
  unidadesPorCaja: z.number().min(1).default(1),
  precio: montoPositivoSchema,
});
...
export const ventaSchema = z.object({
  items: z.array(itemVentaSchema).min(1, { message: 'La venta debe tener al menos un item' }),
...
export const ajusteSchema = z.object({
  productoId: idSchema,
  varianteId: idSchema.optional(),
  cantidad: z.number().refine((v) => v !== 0, { message: 'La cantidad no puede ser cero' }),
  motivo: z.string().min(1, { message: 'Motivo requerido' }).max(200),
  costoPerdida: z.number().min(0).default(0),
});

export const retiroSchema = z.object({
  monto: montoPositivoSchema,
  concepto: z.string().min(1, { message: 'Concepto requerido' }).max(200),
  fecha: fechaIsoSchema,
});
...
export const socioSchema = z.object({
  nombre: nombreSchema,
  porcentaje: z.number().min(0).max(100, { message: 'El porcentaje debe estar entre 0 y 100' }),
});

export const configSchema = z.object({
  nombreNegocio: z.string().max(200).optional(),
  capitalInicial: z.number().min(0).default(0),
  periodoInicio: fechaIsoSchema,
  pin: z.string().max(200).optional(),
  tema: z.enum(['light', 'dark']).optional(),
  webhookUrl: z.string().url().optional().or(z.literal('')),
  webhooksActivos: z.boolean().optional(),
...
/** Valida un objeto contra un schema y devuelve { ok: true } o { ok: false, errors: [...] } */
export function validar(schema, data) {
...
/** Valida un schema parcial (para edicion de campos individuales) */
export function validarParcial(schema, data) {
...
/** Valida un array de items de venta antes de procesar */
export function validarCarrito(items) {
  if (!items || items.length === 0) {
    return { ok: false, errors: ['El carrito esta vacio'] };
  }
  const results = items
    .map((item, idx) => {
      const r = itemVentaSchema.safeParse(item);
      if (!r.success) {
        return { idx, errors: r.error.errors.map((e) => e.message) };
      }
      return null;
    })
    .filter(Boolean);
  if (results.length > 0) {
    return {
      ok: false,
      errors: results.flatMap((r) => r.errors.map((e) => `Item ${r.idx + 1}: ${e}`)),
    };
  }
  return { ok: true };
}

/** Valida que la suma de porcentajes de socios sea 100% */
export function validarSocios(socios) {
  const total = socios.reduce((s, x) => s + n(x.porcentaje), 0);
  if (Math.abs(total - 100) > 0.01) {
    return {
      ok: false,
      errors: [`La suma de porcentajes es ${total.toFixed(1)}%, debe ser 100%`],
    };
  }
  return { ok: true };
}
