...
import { abrirDB, getDB, cerrarDB, guardar, listar, txPut } from '../../src/core/db.js';
...
describe('Integracion - Flujo de venta completo', () => {
  beforeAll(async () => {
    ...
    const db = getDB();
    // Seed datos
    await guardar('productos', { id: 'p1', nombre: 'Producto A', precio: 10, stockMinimo: 5, archivado: false, unidad: '' });
    await guardar('lotes', { id: 'l1', productoId: 'p1', productoNombre: 'Producto A', cantidadInicial: 20, cantidadVendida: 0, costo: 5, fecha: new Date().toISOString() });
  ...
  it('venta reduce stock y actualiza lotes', async () => {
    ...
    await db.transaction('rw', db.ventas, db.lotes, async (trans) => {
      await txPut('ventas', venta, trans);
